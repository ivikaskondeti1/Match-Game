...
class App extends Component {
  state = {
    matchItem: imagesList[Math.floor(Math.random() * imagesList.length)],
    tabSelected: 'FRUIT',
    countValue: 0,
    time: 60,
    scoreCardPage: false,
  ...
  changeTab = event => {
    ...
    this.setState({tabSelected: event.currentTarget.dataset.id})
  }

  checkMatchThumbline = event => {
    const {matchItem} = this.state
    const thumblineId = event.currentTarget.dataset.id
    // console.log(event.currentTarget.dataset.id)
    if (matchItem.id === thumblineId) {
      ...
      this.setState({scoreCardPage: true})
    }
  }
  ...
  render() {
    const {matchItem, tabSelected, countValue, time, scoreCardPage} = this.state
    const filteredList = imagesList.filter(
      eachlist => eachlist.category === tabSelected,
    )
    ...
    return (
      <div className="mainPageCard">
        <div className="headerCard">
          ...
          />
          <div className="headerScoreCard">
            <p className="headerScoreText">Score:</p>
            <p className="headerScoreTextval">{countValue}</p>
            ...
            <img
              ...
              className="Timer"
            />
            <p className="headerScoreTextval">{time} sec</p>
          </div>
        </div>
        {scoreCardPage ? (
          <div className="scoreCardContainer">
            ...
            />
            <h1>YOUR SCORE</h1>
            <p className="resutcount">{countValue}</p>
            <button type="submit" onClick={this.playAgain}>
              <img
                className="reset"
          ...
          <div className="ImageContainer">
            <img src={matchItem.imageUrl} alt="match" className="matchImage" />
            <ul className="tabContainer">
              {tabsList.map(eachtab => (
                <li
                  onClick={this.changeTab}
                  data-id={eachtab.tabId}
                  key={eachtab.tabId}
                  className={
            ...
            <div className="ThmblineContainer">
              {filteredList.map(eachThumbline => (
                <img
                  onClick={this.checkMatchThumbline}
                  data-id={eachThumbline.id}
                  className="thumblineImage"
                  src={eachThumbline.thumbnailUrl}
                  key={eachThumbline.id}
                  alt="thumbnail"
                />
              ))}
            </div>
